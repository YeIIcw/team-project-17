...
public class PreferencesInteractor implements PreferencesInputBoundary {
  ...
  public PreferencesInteractor(QuestionFetcher questionFetcher, GameState gameState,
      PreferencesOutputBoundary presenter, Map<String, Integer> categoryMap) {
  ...
  public void execute(PreferencesInputData input) {
    ...
    try {
      ...
      List<Question> questions = questionFetcher.getQuestions(String.valueOf(categoryId), difficultyParam, typeParam,
          amount);
      ...
      PreferencesOutputData outputData = new PreferencesOutputData(input.getCategory(), input.getDifficulty(),
          input.getType(), input.getNumQuestions(), true, null);
      ...
      PreferencesOutputData outputData = new PreferencesOutputData(input.getCategory(), input.getDifficulty(),
          input.getType(), input.getNumQuestions(), false, "Could not load questions. Please try again.");
      ...
      e.printStackTrace();
      PreferencesOutputData outputData = new PreferencesOutputData(input.getCategory(), input.getDifficulty(),
          input.getType(), input.getNumQuestions(), false, "An error occurred: " + e.getMessage());
      presenter.present(outputData);
    }
  }
