...
public class AppBuilder {
  ...
  private void rebuildCombatStack() {
    ...
    combatPresenter.setGameOverCallback(() -> {
      ...
      // System.out.println("DEBUG: Score saved for " + testUsername + ": " +
      // finalScore);
      //
  ...
  public AppBuilder addPreferencesUseCase() {
    ...
    PreferencesPresenter presenter = new PreferencesPresenter(preferencesViewModel);

    PreferencesInputBoundary interactor = new PreferencesInteractor(questionFetcher, gameState, presenter, categoryMap);

    this.preferencesController = new PreferencesController(interactor);
