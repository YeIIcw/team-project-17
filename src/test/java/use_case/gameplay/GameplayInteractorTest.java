...
class GameplayInteractorTest {
  ...
  void testSubmitAnswer_MovesToNextQuestion() {
    ...
    assertEquals(1, gameState.getCurrentQuestionIndex(), "Question index should advance to 1 after answering");
  ...
  void testSubmitAnswer_MultipleQuestions_ScoreAccumulates() {
    ...
    assertEquals(300, presenter.lastAnswerResult.getTotalScore(), "Total score in output should be 300");
  }
  ...
  void testSubmitAnswer_MixedCorrectIncorrect_OnlyCorrectAddsScore() {
    ...
    assertEquals(100, gameState.getScore(), "Score should remain 100 after one correct and one incorrect answer");
    assertFalse(presenter.lastAnswerResult.isCorrect(), "Last answer should be marked as incorrect");
  }
  ...
  void testSubmitAnswer_LastQuestion_NoMoreQuestionsFlag() {
    ...
    assertFalse(presenter.lastAnswerResult.hasMoreQuestions(), "Should indicate no more questions after last question");
    assertEquals(3, gameState.getCurrentQuestionIndex(), "Should be at index 3 (beyond the last question)");
  }
  ...
  void testGetNextQuestion_ReturnsCurrentQuestion() {
    ...
    assertEquals(1, presenter.questionPresentCallCount, "Presenter should be called once");
    assertNotNull(presenter.lastQuestionPresented, "Presenter should receive output data");
    assertEquals(testQuestions.get(0), presenter.lastQuestionPresented.getCurrentQuestion(),
        "Should present the first question");
    assertEquals(0, presenter.lastQuestionPresented.getTotalScore(), "Initial score should be 0");
    assertTrue(presenter.lastQuestionPresented.hasMoreQuestions(), "Should indicate more questions available");
  }
  ...
  void testGetNextQuestion_AfterAnswering_ReturnsNextQuestion() {
    ...
    assertEquals(100, presenter.lastQuestionPresented.getTotalScore(), "Score should reflect previous correct answer");
  }
  ...
  void testSubmitAnswer_OutputDataContainsCorrectQuestion() {
    ...
    assertNotNull(presenter.lastAnswerResult.getCurrentQuestion(), "Output should contain the answered question");
  ...
  void testGetNextQuestion_WithQuestions_AlwaysTrueHasMoreQuestions() {
    ...
    interactor.getNextQuestion();
    assertTrue(presenter.lastQuestionPresented.hasMoreQuestions(), "Should have more questions at start");

    interactor.submitAnswer(new GameplayInputData(0));
    interactor.getNextQuestion();
    assertTrue(presenter.lastQuestionPresented.hasMoreQuestions(), "Should have more questions after first answer");

    interactor.submitAnswer(new GameplayInputData(2));
    interactor.getNextQuestion();
    assertTrue(presenter.lastQuestionPresented.hasMoreQuestions(), "Should have more questions before last answer");
  }
}
